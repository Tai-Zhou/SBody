	...
	namespace Metric {
		...
		namespace Schwarzschild {
			double dot(const double g[], const double x[], const double y[], const size_t dimension) {
				...
				return -(1. - 2. / g[1]) * x[0] * y[0] + g[1] * x[1] * y[1] / (g[1] - 2.) + gsl_pow_2(g[1]) * x[2] * y[2] + gsl_pow_2(g[1] * sin(g[2])) * x[3] * y[3];
			}
			double ds2(const double x[], const double y[], const size_t dimension) {
				if (dimension == 3)
					return x[1] * gsl_pow_2(x[1] - y[1]) / (x[1] - 2.) + gsl_pow_2(x[1] * (x[2] - y[2])) + gsl_pow_2(x[1] * sin(x[2]) * mod2Pi(x[3] - y[3]));
				return -(1. - 2. / x[1]) * gsl_pow_2(x[0] - y[0]) + x[1] * gsl_pow_2(x[1] - y[1]) / (x[1] - 2.) + gsl_pow_2(x[1] * (x[2] - y[2])) + gsl_pow_2(x[1] * sin(x[2]) * mod2Pi(x[3] - y[3]));
			}
			int qdq2qp(double y[]) {
		...
		namespace Kerr {
			double dot(const double g[], const double x[], const double y[], const size_t dimension) {
				const double r = g[1], r2 = gsl_pow_2(r);
				const double sint = sin(g[2]), sint2 = gsl_pow_2(sint), cost = cos(g[2]), cost2 = gsl_pow_2(cost);
				const double Delta = r2 - 2. * r + a2, rho2 = r2 + a2 * cost2;
			...
			double ds2(const double x[], const double y[], const size_t dimension) {
				...
				const double mr_rho2 = 2. * r / rho2;
				if (dimension == 3)
					return (rho2 / Delta) * gsl_pow_2(x[1] - y[1]) + rho2 * gsl_pow_2(x[2] - y[2]) + ((r2 + a2) * sint2 + 2. * r * a2 * gsl_pow_2(sint2) / rho2) * gsl_pow_2(d3);
			...
			int qdq2qp(double y[]) {
				...
				const double Delta = r2 - 2. * y[1] + a2, rho2 = r2 + a2 * gsl_pow_2(cos(y[2]));
			...
			double energy(const double y[]) {
				return (1. - 2. * y[1] / (gsl_pow_2(y[1]) + a2 * gsl_pow_2(cos(y[2]))) * (1. - a * gsl_pow_2(sin(y[2])) * y[7])) / y[4];
			}
			double energyHamiltonian(const double y[]) {
				return 1. - y[4];
			}
			double angularMomentum(const double y[]) {
				...
				const double mr_rho2 = 2. * y[1] / (r2 + a2 * gsl_pow_2(cos(y[2])));
			...
			int particleNormalization(double y[]) {
				...
				const double Delta = r2 - 2. * r + a2, rho2 = r2 + a2 * gsl_pow_2(cos(y[2]));
				const double mr_rho2 = 2. * r / rho2;
				y[7] += 2. * a * r / (gsl_pow_2(a2r2) - a2 * Delta * sint2);
				y[4] = sqrt(1. - mr_rho2 + 2. * mr_rho2 * a * sint2 * y[7] - (rho2 / (r2 - 2. * r + a2) * gsl_pow_2(y[5]) + rho2 * gsl_pow_2(y[6]) + (a2r2 * sint2 + mr_rho2 * a2 * sint4) * gsl_pow_2(y[7])));
				return std::isnan(y[4]);
			}
			int lightNormalization(double y[], double e) {
		...
		namespace KerrTaubNUT {
			double dot(const double g[], const double x[], const double y[], const size_t dimension) {
				...
				const double Delta = r2 - 2. * r - l2 + a2, rho2 = r2 + gsl_pow_2(l + a * cost), chi = a * sint2 - 2. * l * cost;
				const double rho_2 = 1. / rho2;
				if (dimension == 3)
					return (rho2 / Delta) * x[1] * y[1] + rho2 * x[2] * y[2] + (gsl_pow_2(rho2 + a * chi) * sint2 - chi * chi * Delta) * rho_2 * x[3] * y[3];
				return (a2 * sint2 - Delta) * rho_2 * x[0] * y[0] - 2. * ((r + l2) * a * sint2 + Delta * l * cost) * rho_2 * (x[0] * y[3] + x[3] * y[0]) + (rho2 / Delta) * x[1] * y[1] + rho2 * x[2] * y[2] + (gsl_pow_2(rho2 + a * chi) * sint2 - chi * chi * Delta) * rho_2 * x[3] * y[3];
			}
			double ds2(const double x[], const double y[], const size_t dimension) {
				...
				return (a2 * sint2 - Delta) * rho_2 * gsl_pow_2(d0) - 4. * ((r + l2) * a * sint2 + Delta * l * cost) * rho_2 * d0 * d3 + (rho2 / Delta) * gsl_pow_2(x[1] - y[1]) + rho2 * gsl_pow_2(x[2] - y[2]) + (gsl_pow_2(rho2 + a * chi) * sint2 - gsl_pow_2(chi) * Delta) * rho_2 * gsl_pow_2(d3);
			}
			int qdq2qp(double y[]) {
				...
				y[5] *= rho2 / Delta * dtdtau;
				y[6] *= rho2 * dtdtau;
				y[7] = (-2. * ((r + l2) * a * sint2 + Delta * l * cost) + (gsl_pow_2(r2 + l2 + a2) * sint2 - gsl_pow_2(a * sint2 - 2. * l * cost) * Delta) * y[7]) * rho_2 * dtdtau;
				return 0;
			}
			int qp2qdq(double y[]) {
				...
				y[5] *= Delta * rho_2 * y[4];
				y[6] *= rho_2 * y[4];
				y[7] = (-2. * ((r + l2) * a * sint2 + Delta * l * cost) * pt + (Delta - a2 * sint2) * y[7]) / (Delta * rho2 * sint2) * y[4];
				return 0;
			}
			int function(double t, const double y[], double dydt[], void *params) {
				dydt[0] = y[4]; // d\tau/dt
			...
			int particleNormalization(double y[]) {
				...
				y[4] = sqrt(((Delta - a2 * sint2) + 4. * ((r + l2) * a * sint2 + Delta * l * cost) * y[7] - (gsl_pow_2(r2 + l2 + a2) * sint2 - gsl_pow_2(a * sint2 - 2. * l * cost) * Delta) * gsl_pow_2(y[7])) / rho2 - rho2 * (gsl_pow_2(y[5]) / Delta + gsl_pow_2(y[6])));
				return std::isnan(y[4]);
			}
			int lightNormalization(double y[], double e) {
				const double r = y[1], r2 = gsl_pow_2(r);
